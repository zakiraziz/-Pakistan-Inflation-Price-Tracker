...
MAX_PCT_JUMP = {"Grocery": 20, "Fresh produce": 60, "Energy": 15,
                "Poultry": 20, "Dairy & protein": 18}
MAX_PCT_DROP = {"Grocery": 25, "Fresh produce": 45, "Energy": 20,
                "Poultry": 25, "Dairy & protein": 22}
DEFAULT_JUMP = 20.0
DEFAULT_DROP = 25.0


def category_limits(category: str):
    return (MAX_PCT_JUMP.get(category, DEFAULT_JUMP),
            MAX_PCT_DROP.get(category, DEFAULT_DROP))
...
def classify(category: str, price: float, prev_price: float = None):
    ...
    if pct > max_jump:
        return db.STATUS_PENDING, (
            f"suspicious +{pct:.1f}% week-over-week (max {max_jump:.0f}%)")
    if pct < -max_drop:
        return db.STATUS_PENDING, (
            f"suspicious {pct:.1f}% week-over-week (min -{max_drop:.0f}%)")
    return db.STATUS_APPROVED, ""