...
def _seed_items(con):
    con.execute("INSERT INTO items (id, name, category, unit) "
                "VALUES (1, 'Rice', 'Grocery', 'per kg')")
    con.commit()
...
def test_provenance_is_recorded(tmp_db):
    con, _ = tmp_db
    _seed_items(con)
    points = [{"item": "Rice", "date": "2026-01-01", "price": 100.0,
               "method": "csv-import"}]
...
def test_suspicious_point_is_held_pending_and_excluded(tmp_db):
    ...
    row = con.execute("SELECT status, review_note FROM prices "
                      "WHERE date = '2026-01-08'").fetchone()
...
def test_unknown_item_is_rejected(tmp_db):
    con, _ = tmp_db
    _seed_items(con)
    counts = ingest(con, [{"item": "Mystery", "date": "2026-01-01",
                           "price": 10.0, "method": "csv"}],
                    source_name="csv", method="csv-import")
...
def test_ingestion_is_audited(tmp_db):
    con, _ = tmp_db
    _seed_items(con)
    ingest(con, [{"item": "Rice", "date": "2026-01-01", "price": 100.0,
                  "method": "csv"}], source_name="csv", method="csv-import")
