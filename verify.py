...
def q(**kw):
    ...
    return "?" + urllib.parse.urlencode(
        {k: str(v) for k, v in kw.items() if v not in (None, "")}
    )
...
for path in ["/", "/static/style.css", "/static/helpers.js",
             "/static/app.js", "/static/explainer.html"]:
...
if j:
    ...
    ok("api/items includes Wheat flour",
       any("flour" in n.lower() for n in names))
...
if j:
    ...
    ok("api/series rows have name/date/price",
       all({"name", "date", "price"} <= set(r) for r in j))
    ok("api/series price is numeric",
       all(isinstance(r["price"], (int, float)) for r in j))
...
if j:
    ...
    ok("api/series windowed respects end date",
       all(r["date"] <= "2026-03-01" for r in j))
log("\n--- /api/index ---")
...
if j:
    ...
    ok("api/index normalised to 100 at window start",
       j[0].get("index") == 100.0, str(j[0].get("index")))
    ok("api/index ends above 100", j[-1].get("index", 0) > 100,
       str(j[-1].get("index")))
    ok("api/index rows have date+index",
       all({"date", "index"} <= set(r) for r in j))
    ok("api/index is monotonic in date",
       [r["date"] for r in j] == sorted(r["date"] for r in j))

log("\n--- /api/metrics ---")
j, e = get_json("/api/metrics" + q(start=START))
ok("GET /api/metrics?start=2023-01-01 200", j is not None, e)
if j:
    for field in ["count", "weeks", "first_date", "last_date", "basket_pct",
                  "start_total", "end_total", "biggest_riser", "biggest_faller"]:
...
if j:
    ...
    ok("pivot.items == 11", len(j.get("items", [])) == 11,
       str(len(j.get("items", []))))
    ...
    if j.get("items"):
        ...
        ok("pivot.item.last == prices[-1]",
           it.get("last") == it.get("prices", [None])[-1])
...
if j:
    ok("pivot honours items filter (3 items)",
       len(j.get("items", [])) == 3, str(len(j.get("items", []))))
...
if j is not None:
    ...
    ok("api/alerts rows have required fields",
       all({"name", "category", "date", "price", "prev_price", "pct"} <= set(r)
           for r in j))
...
if j is not None:
    ok("api/alerts windowed respects start",
       all(r.get("date", "") >= RECENT for r in j))
...
if status == 200:
    ...
    ok("csv header is item,date,price", text.startswith("item,date,price"),
       text.splitlines()[0] if text.splitlines() else "")
    ok("csv has data rows", len(text.splitlines()) > 1,
       str(len(text.splitlines())) + " lines")
...
for marker in ['id="kpis"', 'id="view"', 'id="alertsPanel"', 'id="itemList"',
               'id="categoryFilters"', 'id="search"', 'id="threshold"',
               'id="start"', 'id="end"', 'id="apply"', 'id="export"',
               'id="updateNow"', 'id="updatedBadge"', 'id="toast"',
               'id="infoLine"', 'role="tablist"', 'class="tab"',
               'class="hero"', 'class="presets"', 'class="kpi"',
               "/static/helpers.js", "/static/app.js", "/static/style.css",
               "chart.js", "Inter"]:
...
ok("style.css defines >= 40 class selectors", len(css_classes) >= 40,
   str(len(css_classes)))
...
for name in ["renderSkeletons", "renderAlerts", "renderTrends", "renderCompare",
             "renderData", "renderError", "money", "pct", "pctCls", "esc",
             "monthsAgo", "thresholdVal", "destroyChart", "el", "toast"]:
...
try:
    ...
    req = urllib.request.Request(
        BASE_URL + "/ingest/next", data=payload,
